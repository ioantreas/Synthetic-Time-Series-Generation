...
def main(config, log_dir):
    ...
    train = np.load(config["train_dataset"])
    test = np.load(config["test_dataset"])

    assert train.ndim == 3
    assert test.ndim == 3

    assert train.shape[1] == context_length + prediction_length
    assert test.shape[1] == context_length + prediction_length

    train_dataset = ListDataset(
        [{"target": x.T, "start": "2000-01-01"} for x in train],
        freq=freq,
    )

    test_dataset = ListDataset(
        [{"target": x.T, "start": "2000-01-01"} for x in test],
        freq=freq,
    )
    ...
    if config["use_validation_set"]:
        ...
        train_dataset = ListDataset(
            [{"target": x.T, "start": 0} for x in train[:n_train]],
            freq=freq,
        )
    ...
    metrics = (
        evaluate_conditional(config, model, test_dataset, transformation)
