...
def client_get_partial_guidance_latent(
    ...
    # z_weight = z_weight * (z_weight > 0.50).float() * 1 + z_weight * (z_weight <= 0.50).float() * 0.001
    ...
    # keep top X% of latent weights unchanged
    threshold = torch.quantile(z_weight.flatten(), 1.0 - args.latent_keep_percent)

    keep_mask = z_weight >= threshold

    print(
        f"Keeping {(keep_mask.float().mean().item() * 100):.2f}% "
        f"of latent weights unchanged (threshold={threshold.item():.4f})"
    )

    z_weight = torch.where(keep_mask, z_weight, z_weight * 0.001)
...
def build_hybrid_guidance(x_full, x_obs, mask):
    ...
    if args.no_local_guidance:
        print("--------------\nNO LOCAL GUIDANCE\n--------------")
        z_target_full[:, :, target_latent_idx] = 0
        z_weight_full[:, :, target_latent_idx] = 0
    else:
