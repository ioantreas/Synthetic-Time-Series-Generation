...
def guidance_fn(x_recon, x_latent, t):
    B, T, C = x_recon.shape
    target = torch.zeros_like(x_recon)

    # raise the middle third
    a = T // 3
    b = 2 * T // 3
    target[:, a:b, :] = 2.0

    return ((x_recon - target) ** 2).mean()
...
def sample_guided(model, decoder, num_samples, seq_len, num_channels, device, guidance_scale=0.5):
    ...
    for i in reversed(range(model.timesteps)):
        ...
        sqrt_one_minus_ab_prev = torch.sqrt(1.0 - alpha_bar_prev)

        if i > 0:
            x = sqrt_ab_prev * x0_guided + sqrt_one_minus_ab_prev * eps
        else:
            x = x0_guided
