...
def estimate_latent_observability(
    ...
    for b in range(B):
        for c in range(C):
            ...
            x_init[b, :, c] = torch.tensor(
                interp,
                device=x_init.device
            )

    # ----------------------------------------------------
    # latent of interpolation init
    # ----------------------------------------------------

    with torch.no_grad():

        z_init = ae.encode(
            x_init.permute(0, 2, 1)
        )

        # TODO
        if z_init.shape[1] > z_init.shape[2]:
            z_init = z_init.permute(0, 2, 1)

    # ----------------------------------------------------
    # stochastic latent deltas
    # ----------------------------------------------------
    ...
    with torch.no_grad():
        ...
        for _ in range(num_samples):
            ...
            rand_fill = noise_scale * torch.randn_like(x_rand)

            rand_fill = rand_fill.clamp(
                -noise_clip,
                noise_clip
            )
            ...
            # TODO
            if z_rand.shape[1] > z_rand.shape[2]:
                z_rand = z_rand.permute(0, 2, 1)

            delta = (z_rand - z_init).abs()
    ...
    # ----------------------------------------------------
    # average stochastic sensitivity
    # ----------------------------------------------------

    delta_mean = deltas.mean(dim=0)

    # normalize globally per sample
    delta_norm = delta_mean / (
        delta_mean.mean(dim=(1,2), keepdim=True) + 1e-8
    )
    ...
    weight = 1.0 / (1.0 + temperature * delta_norm)

    weight = weight / (
        weight.amax(dim=(1,2), keepdim=True) + 1e-8
    )
...
def client_get_guidance_latent(
    ...
    # transformer encode returns [B,L,D]
    with torch.no_grad():
    ...
    for _ in range(steps):
        ...
        # transformer decode returns [B,C,T]
    ...
    with torch.no_grad():
        ...
        if plot:
            ...
            for ch in range(num_plot):
                ...
                plt.title(f"Client anchor channel {ch}")
                plt.legend()
    ...
    # -------------------------
    # Weight method 1: fast interpolation
    # -------------------------
    ...
    # -------------------------
    # Weight method 2: stochastic observability
    ...
    print(z_weight)
    z_weight = z_weight * (z_weight > 0.15).float() * 1.0 + z_weight * (z_weight <= 0.15).float() * 0.0005
    low_idx = (z_weight[0] <= 0.1).nonzero(as_tuple=False)

    print("\n=== LOW WEIGHT INDICES (sample 0) ===")

    for idx in low_idx:
        l = idx[0].item()
        d = idx[1].item()
        val = z_weight[0, l, d].item()

        print(f"latent_step={l:02d}  dim={d:03d}  weight={val:.6f}")


    print("z_weight mean:", z_weight.mean().item())
...
def refine_latent(z_full, steps, scale, z_guidance_target, z_guidance_weights):
    ...
    for _ in range(steps):
        ...
        grad = grad / (
                grad.norm(dim=(1, 2), keepdim=True) + 1e-8
        )
...
def compute_masked_metrics(x_pred, x_true, mask):
    ...
    if missing.sum() == 0:
        ...
        metrics["spectral_mse_missing"] = float(
            ((pred_fft - true_fft)[mask == 0] ** 2).mean()
        )

    metrics["mse_observed"] = (
        np.nan if observed.sum() == 0
        else float(((x_pred - x_true)[observed] ** 2).mean())
    )
...
def compute_full_metrics(x_pred, x_true):
    metrics = {
        "mse_full": float(((x_pred - x_true) ** 2).mean()),
        "mae_full": float(np.abs(x_pred - x_true).mean()),
        "corr_full": float(safe_corr(x_pred.flatten(), x_true.flatten())),
    }
...
def run_scenario(name, mask_np, out_dir):
    ...
    if len(hidden_local_idx) > 0:
        x_pred_hidden = x_pred[:, :, hidden_local_idx]
        x_true_hidden = x_true[:, :, hidden_local_idx]
        x_ae_hidden = x_ae[:, :, hidden_local_idx]
    else:
    ...
    hidden_metrics = (
        compute_full_metrics(x_pred_hidden, x_true_hidden)
        if len(hidden_local_idx) > 0 else {}
    )

    guided_metrics_ae_target = compute_masked_metrics(
        x_pred_guided,
        x_ae_guided,
        mask_guided,
    )
    ...
    for ch in deterministic_plot_channels(hidden_local_idx, k=5):
        plt.figure(figsize=(10, 4))
...
def main():
    ...
    model = TSDiff.load_from_checkpoint(ckpt_path, weights_only=False).to(device)
    ...
    if start != num_channels:
        raise ValueError(
            f"Latent mismatch: diffusion model expects {num_channels}, "
    ...
    # this is the original feature dimension, not diffusion latent dim
